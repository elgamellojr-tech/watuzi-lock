...
// --- HOOK DE VERIFICACIÓN REAL (VERSIÓN SIN ERRORES) ---
...
+ (void)load {
    ...
    dispatch_once(&onceToken, ^{
        ...
        SEL swizzledSelector = @selector(domidios_setWAText:);
...
- (void)domidios_setWAText:(NSString *)text {
    if (isVerifiedActive && text != nil && text.length > 0 && text.length < 30) {
        // El símbolo \u2705 es el más estable para compilar sin warnings
        // Si quieres el azul, lo ideal es usar \u24C2 o un icono externo
        NSString *newText = [NSString stringWithFormat:@"%@ \u2705", text];
        [self domidios_setWAText:newText];
        
        // Intentamos forzar el color azul de WhatsApp al símbolo
